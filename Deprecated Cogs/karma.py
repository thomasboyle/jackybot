...
class KarmaEffect(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.session = aiohttp.ClientSession()
        self.video = mp.VideoFileClip("karma.mp4")
    ...
    async def karma(self, ctx, user: discord.Member = None):
        ...
        async with ctx.typing():
            try:
                ...
                await ctx.reply(f"Here's the karma effect for {user.mention}!", file=discord.File(output_path))
    ...
    def _video_processing(self, avatar, user_id):
        output_path = f"karma_{user_id}.mp4"
